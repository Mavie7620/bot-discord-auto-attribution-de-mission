...
TAILLE_MAX_DISCORD = 8 * 1024 * 1024  # 8 Mo (limite d'upload standard)

def generer_backup_complet():
    """Construit la sauvegarde complete (missions actives + fichiers disques).
    Retourne (buffer, nom_fichier, taille_octets). Utilise par /total_backup ET la sauvegarde auto."""
    donnees_globales = {
        "missions_actives": {},
        "fichiers_disques": {}
    }
...
async def sauvegarde_automatique():
    """Sauvegarde complete automatique toutes les 2 heures, envoyee dans le salon dedie."""
    buffer = None
    try:
        ...
        if salon is None:
            try:
                ...
                salon = None
        if salon is None:
            print(f"[Sauvegarde auto] Salon {SALON_BACKUP_AUTO_ID} introuvable. Sauvegarde annulÃ©e.")
            return
        ...
        if taille > TAILLE_MAX_DISCORD:
            message_erreur = (
                f"âš ï¸ **Sauvegarde automatique impossible** : le fichier fait "
                f"{taille / (1024 * 1024):.2f} Mo, ce qui dÃ©passe la limite d'envoi Discord."
            )
            print(f"[Sauvegarde auto] {message_erreur}")
            try:
                await salon.send(message_erreur)
            except Exception as e:
                print(f"[Sauvegarde auto] Impossible de prÃ©venir le salon : {e}")
            return

        await salon.send(
            content=f"ðŸ—„ï¸ **Sauvegarde automatique** â€” {datetime.now().strftime('%d/%m/%Y %H:%M')}",
            file=discord.File(buffer, filename=nom_fichier)
        )
        print(f"[Sauvegarde auto] Sauvegarde envoyÃ©e : {nom_fichier} ({taille} octets)")
    except Exception as e:
        print(f"[Sauvegarde auto] Erreur pendant la sauvegarde : {e}")
    finally:
        # LibÃ©ration mÃ©moire / nettoyage du fichier temporaire
        try:
